 ...
 * Mirrors the student-form array pattern; the array is stored as `string[]`.
 */
      ...
      render={({ field, fieldState }) => {
        ...
        return (
          <Field label={label} htmlFor={id} hint={hint} error={fieldState.error?.message}>
            <div className="med-chips" data-invalid={fieldState.invalid || undefined}>
              {items.map((item, i) => (
                <span className="med-chip" key={`${item}-${i}`}>
                  <span className="med-chip__label">{item}</span>
                  <button
                    type="button"
